...
class Nginx < Formula
  ...
  def fetch_from_github(name)
      name, respository = name.split('/')
      raise "You must specify a repository name for github modules" if respository.nil?
      puts "- cloning #{respository} from github..."
      path = "/tmp/nginx-modules/#{name}/#{respository}"
      `rm -rf #{path}`
      `git clone -q git://github.com/#{name}/#{respository} #{path}`
      return path
  end

  def fetch_from_local(path)
      raise "Unable to find locate module in directory #{path}" if !File.directory? (path)
      return path
  end

  def add_from_local; collect_modules(/^--with-module=/); end
  def add_from_github; collect_modules(/^--with-github-module=/); end
  ...
  def install
    ...
    add_from_local.each {|name| args << "--add-module=#{fetch_from_local(name)}"}
